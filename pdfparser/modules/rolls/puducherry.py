...
class PuducherryPatterns(Patterns):
	...
	box_patterns = dict(
		main=r'([A-Z]{2,5}/?[0-9]{2,}[0-9/]*(.*?(?=[A-Z]{2,5}/?[0-9]{2,}[0-9/]*)|.*))',
		deletion=None
	)
	...
	general_patterns = dict(
		...
		stationAddress=r'\nAddress of Polling\nStation\n:\n([^:]*)',
		netMale=r'Net Electors in the Part\n(\d+)',
		netFemale=r'Net Electors in the Part\n\d+\n(\d+)',
		netThird=r'Net Electors in the Part\n\d+\n\d+\n(\d+)',
		netTotal=r'Net Electors in the Part\n\d+\n\d+\n\d+\n(\d+)'
	)
